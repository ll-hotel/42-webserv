...
const std::string &status_table(t_status_code code)
{
	...
	if (status.empty()) {
		...
		status[SUCCESS_NOT_AUTHORITATIVE_INFORMATION] =
		...
		status[HTTP_ERROR_REQUEST_ENTITY_TOO_LARGE] =
			"Request entity too large";
		status[HTTP_ERROR_REQUEST_URI_TOO_LONG] =
			"Request uri too long";
		status[HTTP_ERROR_UNSUPPORTED_MEDIA_TYPE] =
			"Unsupported media type";
		status[HTTP_ERROR_REQUESTED_RANGE_UNSATISFIABLE] =
		...
		status[HTTP_ERROR_BAD_MAPPING_OR_MISDIRECTED_REQUEST] =
			"Bad mapping or misdirected request";
		status[HTTP_ERROR_UNPROCESSABLE_ENTITY] =
		...
		status[HTTP_ERROR_REQUEST_HEADER_FIELDS_TOO_LARGE] =
			"Request header fields too large";
		status[HTTP_ERROR_RETRY_WITH] = "Retry with";
		status[HTTP_ERROR_BLOCKED_BY_WINDOWS_PARENTAL_CONTROL] =
			"Blocked by windows parental control";
		status[HTTP_ERROR_UNAVAILABLE_FOR_LEGAL_REASONS] =
			"Unavailable for legal reasons";
		status[HTTP_ERROR_UNRECOVERABLE_ERROR] = "Unrecoverable error";

		status[HTTP_ERROR_NO_RESPONSE] = "No response";
		status[HTTP_ERROR_SSL_CERTIFICATE_ERROR] =
			"sSL certificate error";
		status[HTTP_ERROR_SSL_CERTIFICATE_REQUIRED] =
			"sSL certificate required";
		status[HTTP_ERROR_HTTP_REQUEST_SENT_TO_HTTPS_PORT] =
			"Http request sent to https port";
		status[HTTP_ERROR_TOKEN_EXPIRED_OR_INVALID] =
			"Token expired or invalid";
		status[HTTP_ERROR_CLIENT_CLOSED_REQUEST] =
			"Client closed request";

		status[SERVER_ERROR_INTERNAL_SERVER_ERROR] =
			"Internal server error";
		status[SERVER_ERROR_NOT_IMPLEMENTED] = "Not implemented";
		status[SERVER_ERROR_BAD_GATEWAY_OR_PROXY_ERROR] =
			"Bad gateway or proxy error";
		status[SERVER_ERROR_SERVICE_UNAVAILABLE] =
			"Service unavailable";
		status[SERVER_ERROR_GATEWAY_TIMEOUT] = "Gateway timeout";
		status[SERVER_ERROR_HTTP_VERSION_NOT_SUPPORTED] =
			"Http version not supported";
		status[SERVER_ERROR_VARIANT_ALSO_NEGOTIATE] =
			"Variant also negotiate";
		status[SERVER_ERROR_INSUFFICIENT_STORAGE] =
			"Insufficient storage";
		status[SERVER_ERROR_LOOP_DETECTED] = "Loop detected";
		status[SERVER_ERROR_BANDWITH_LIMIT_EXCEEDED] =
			"Bandwith limit exceeded";
		status[SERVER_ERROR_NOT_EXTENDED] = "Not extended";
		status[SERVER_ERROR_NETWORK_AUTH_REQUIRED] =
