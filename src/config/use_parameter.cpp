...
void change_root(const Parameter &parameter, std::string *root)
{
	...
	*root = canonicalize(parameter.second[0]);
}
...
void change_index_page(const Parameter &parameter, std::string *index_page)
{
	...
	*index_page = canonicalize(parameter.second[0]);
}
		       ...
		       std::map<int, std::string> *error_pages)
{
	...
	(*error_pages)[code] = canonicalize(parameter.second[1]);
}
...
{
	if (parameter.second.size() > 1)
		WS_THROW("too many arguments for upload_dir");
	*upload_dir = canonicalize(parameter.second[0]);
}
